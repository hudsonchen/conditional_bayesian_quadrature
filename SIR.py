...
def SIR(args, rng_key):
    ...
    # beta_lab_list = [0.40, 0.41, 0.42, 0.43, 0.44, 0.45]
    Nx = 10
    beta_lab_array = jax.random.uniform(rng_key, shape=(Nx, ), minval=0.3, maxval=0.6)
    beta_lab_all = jnp.linspace(0.3, 0.6, 100)
    gamma_lab = 0.05
    rate = 100.0
    scale = 1. / rate
    T = 150
    ...
    if args.mode == 'peak_number':
        ...
        pass
    if not os.path.exists(f'{args.data_path}/SIR/peak_infected_time_array.npy') or not os.path.exists(f'{args.data_path}/SIR/peak_infected_number_array.npy'):
    ...
    for j in tqdm(range(Nx)):
        ...
        for Ny in Ny_list:
            ...
            psi_mean, psi_std = Bayesian_Monte_Carlo(rng_key, beta_array, f_beta_array_standardized, d_log_beta_array, stein_Gaussian)
            psi_mean = psi_mean * f_beta_array_std + f_beta_array_mean
            psi_std = psi_std * f_beta_array_std
            psi_mean_array = psi_mean_array.at[j].set(MC_large_sample)
    ...
    BMC_mean, BMC_std = GP(psi_mean_array[:, None], psi_std_array[:, None],
                           beta_lab_array[:, None], beta_lab_all[:, None])
